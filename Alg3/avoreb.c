...
static void enfileirar(fila *f, struct nodo *ptr, int32_t nivel) {
...
static void desenfileirar(fila *f, struct nodo** ptr, int32_t *nivel) {
  item_fila *rem = f->ini;
...
static void destruir_fila(fila *f) {
  while (!fila_vazia(f)) {
    ...
    int32_t nivel = 0;
    desenfileirar(f, &ptr, &nivel);
  }
  free(f);
}

static struct nodo *alocar_nodo(int32_t t_arvore) {
  struct nodo* novo = malloc_check(sizeof(struct nodo));

  novo->chaves = malloc_check((size_t)(2 * t_arvore - 1) * sizeof(int32_t));
  novo->filhos = malloc_check((size_t)(2 * t_arvore) * sizeof(struct nodo *));
  novo->n = 0;
  novo->ehfolha = true;
...
static void destruir_nodos(struct nodo *x) {
  if (x == NULL) {
    return;
  }
...
static struct nodo *buscar_nodo(struct nodo *x, int32_t chave, int32_t *idx_encontrado) {
  int32_t i = 0;
...
static void dividir_filho(struct nodo *x, int32_t i, int32_t t_arvore) {
  struct nodo *y = x->filhos[i];
  struct nodo *z = alocar_nodo(t_arvore);
...
static void inserir_nao_cheio(struct nodo *x, int32_t chave, int32_t t_arvore) {
  int32_t i = x->n - 1;
...
static void imprimir_em_ordem(struct nodo *x, bool *primeiro) {
  if (x == NULL) {
    return;
  }
...
static void imprimir_nodo(const struct nodo *x) {
